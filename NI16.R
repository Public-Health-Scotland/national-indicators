...
ni16_connect <- dbConnect(odbc::odbc(),
  dsn = "SMRA",
...
ni16_extract <-
  ...
  dplyr::mutate(dplyr::across(main_condition:other_condition_5, str_sub, 1, 3)) %>%
...
dplyr::mutate(falls = TRUE) %>%
  # Extract the financial month and year from discharge date
  fin_year_month(discharge_date) %>%
  dplyr::select(-c(main_condition:other_condition_5))

ni16 <- dplyr::left_join(ni16_extract,
  get_pc_lookup(c("pc7", "ca2018", "datazone2011")),
  by = c("postcode" = "pc7")
) %>%
  dplyr::filter(!datazone2011 == "") %>%
  dplyr::left_join(
...
test <- ni16 %>%
  ...
  tidyr::pivot_longer(c("month", "temp_month"),
    values_to = "month"
  ) %>%
  dplyr::select(-name) %>%
  ...
  tidyr::pivot_longer(c("locality", "temp_loc"), values_to = "locality") %>%
  dplyr::select(-name) %>%
  # Scotland totals
  dplyr::mutate(temp_part = dplyr::if_else(partnership == "Clackmannanshire & Stirling",
    NA_character_,
    "Scotland"
  )) %>%
  tidyr::pivot_longer(
...
ni16_final <- dplyr::left_join(test,
  get_loc_pops(),
...
scot <- ni16_final %>%
  ...
  dplyr::select(year1, data1, scotland)
ni16_final <- dplyr::left_join(ni16_final, scot, by = c("year1", "data1"))
