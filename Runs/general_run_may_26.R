...
smra_data <- process_ni14_smra_extract(min_date = "01-APR-2015")
nrs_data <- process_ni14_nrs_extract(min_date = "01-APR-2015")
...
#get_ni_output_dir()
#fs::path(get_ni_output_dir(), "ni14_smra_temp.parquet")
#fs::file_exists(fs::path(get_ni_output_dir(), "ni14_smra_temp.parquet"))
...
arrow::write_parquet(ni14_with_scot, fs::path(get_ni_output_dir(), "ni14_tableau_output_jul_26.parquet"))

ni14_quarterly_xl <- ni14_quarterly %>%
  dplyr::filter(!(year %in% c("2015/16", "2026/27")) & locality == "All") %>%
...
ni14_cal_xl <- ni14_cal_annual %>%
  dplyr::filter(year != "2015" & year != "2026") %>%
...
arrow::write_parquet(ni14_xl, fs::path(get_ni_output_dir(), "ni14_excel_output_jul_26.parquet"))
#write_csv(ni14_xl, "/conf/sourcedev/TableauUpdates/Ryan/ni14_excel_output_may_26.csv")
...
ni15_raw <- calculate_ni15(extract_start = "01-APR-2016", extract_end = "30-APR-2026")

#ni15_raw <- calculate_ni15
...
ni15_cal <- ni15_cal %>%
  dplyr::rename(partnership = ca2019name, locality = hscp_locality) %>%
  dplyr::filter(time_period == "Annual" & !(year %in% c("2015", "2026"))) %>%
  dplyr::mutate(year = as.character(year))
...
#for july pub - ignore may release
#ni15_xl <- filter(ni15_xl, year != "2026/27")
#ni15_xl <- filter(ni15_xl, year != "2017" & year != "2017/18" & year != "2018" & year != "2019" & year != "2020" & year != "2021" & year != "2022" & year != "2023" & year != "2024" & year != "2025")

arrow::write_parquet(ni15_xl, fs::path(get_ni_output_dir(), "ni15_excel_output_jul_26.parquet"))
...
arrow::write_parquet(ni15_tableau, fs::path(get_ni_output_dir(), "ni15_tableau_output_jul_26.parquet"))

# NI18
# Nice and easy here
ni18 <- calculate_ni18(fs::path(get_ni_input_dir(), "NI18/2025-04-11-balance-of-care.xlsm"), min_year = 2015)

## NI12 - scroll to line 235 ##
...
calculate_ni12_from_slf <- function(
    slf,
    fin_year_start,
    fin_year_end
) {

  # Flag admission records (same as original)
  slf_flagged <- slf %>%
    dplyr::mutate(
      admission_record_flag =
        flag_admission_records(record_keydate1, fin_year_start, fin_year_end),
      record_keydate1 =
        dplyr::if_else(record_keydate1 < fin_year_start, fin_year_start, record_keydate1)
    ) %>%
    dplyr::mutate(
  ...
  fin_year_totals <- slf_flagged %>%
    dplyr::group_by(fin_year, partnership, hscp_locality, fin_month) %>%
    dplyr::summarise(admissions = sum(admission_record_flag), .groups = "drop") %>%
    dplyr::rename(month = fin_month, year = fin_year) %>%
    dplyr::mutate(pop_year = stringr::str_sub(year, 1, 4))

  # Calendar-year totals
  cal_year_totals <- slf_flagged %>%
    dplyr::group_by(cal_year, partnership, hscp_locality, cal_month) %>%
    dplyr::summarise(admissions = sum(admission_record_flag), .groups = "drop") %>%
    dplyr::rename(month = cal_month, year = cal_year) %>%
    dplyr::mutate(pop_year = as.character(lubridate::year(fin_year_start)))

  # Apply NI12 group expansions (All, Scotland, Clacks+Stirling, Annual)
  fin_year_totals <- add_all_groupings_ni12(fin_year_totals)
  cal_year_totals <- add_all_groupings_ni12(cal_year_totals)

  # Attach population for rates
  pop_lookup <- read_population_lookup(
    ages_required = "over18",
    type = "locality"
  )

  fin_year_totals <- fin_year_totals %>%
    dplyr::left_join(pop_lookup,
                     by = c("hscp_locality" = "locality",
                            "pop_year",
                            "partnership")
    ) %>%
    dplyr::mutate(value = admissions / over18_pop * 100000)

  cal_year_totals <- cal_year_totals %>%
    dplyr::left_join(pop_lookup,
                     by = c("hscp_locality" = "locality",
                            "pop_year",
                            "partnership")
  ...
  # Final output list (matches original calculate_ni12)
  return(
    list(
      fin_year_totals = fin_year_totals,
      cal_year_totals = cal_year_totals
...
slf <- arrow::read_parquet(slf_path,
  ...
  # NEW
  dplyr::left_join(
    readr::read_rds(get_locality_path()),   # adds hscp_locality + ca2018
    by = "datazone2011"
  ) %>%
  dplyr::mutate(
    partnership = phsmethods::match_area(ca2018)   # create partnership
  )

# ni12 <- calculate_ni12(slf, fin_year_start = ymd("2019-04-01"), fin_year_end = ymd("2020-03-31"))
data <- calculate_ni12(2526, write_to_disk = FALSE)
fin_yearni12 <- as_tibble(data[["fin_year_totals"]])

# NEW
ni12 <- calculate_ni12_from_slf(
  slf,
  fin_year_start = ymd("2025-04-01"),
  fin_year_end   = ymd("2026-03-31")
)

arrow::write_parquet(
  ni12[["fin_year_totals"]],
  glue::glue("{get_ni_output_dir()}/ni12_20{year}_financial_year.parquet")
)

arrow::write_parquet(
  ni12[["cal_year_totals"]],
  glue::glue("{get_ni_output_dir()}/ni12_20{year}_calendar_year.parquet")
)
...
ni12_add <- dplyr::bind_rows(arrow::read_parquet(fs::path(get_ni_output_dir(), "ni12_202021_financial_year.parquet")),
                             ...
                             arrow::read_parquet(fs::path(get_ni_output_dir(), "ni12_202526_financial_year.parquet")))
...
ni12_add_cal <- dplyr::bind_rows(arrow::read_parquet(fs::path(get_ni_output_dir(), "ni12_202021_calendar_year.parquet")),
                                 ...
                                 arrow::read_parquet(fs::path(get_ni_output_dir(), "ni12_202526_calendar_year.parquet")))

ni12_cal <- ni12_add_cal %>%
  dplyr::filter(month != "Annual" & !(year %in% c("2017", "2027"))) %>%
...
arrow::write_parquet(ni12_xl, fs::path(get_ni_output_dir(), "ni12_excel_output_jul_26.parquet"))
...
arrow::write_parquet(ni12_tableau, fs::path(get_ni_output_dir(), "ni12_tableau_output_jul_26.parquet"))
...
ni13_add <- dplyr::bind_rows(arrow::read_parquet(fs::path(get_ni_output_dir(), "ni13_202021_financial_year.parquet")),
                             ...
                             arrow::read_parquet(fs::path(get_ni_output_dir(), "ni13_202526_financial_year.parquet")))
...
ni13_add_cal <- dplyr::bind_rows(arrow::read_parquet(fs::path(get_ni_output_dir(), "ni13_202021_calendar_year.parquet")),
                                 ...
                                 arrow::read_parquet(fs::path(get_ni_output_dir(), "ni13_202526_calendar_year.parquet")))

ni13_cal <- ni13_add_cal %>%
  dplyr::filter(!(year %in% c("2017", "2027"))) %>%
...
arrow::write_parquet(ni13_xl, fs::path(get_ni_output_dir(), "ni13_excel_output_jul_26.parquet"))
...
arrow::write_parquet(ni13_tableau, fs::path(get_ni_output_dir(), "ni13_tableau_output_jul_26.parquet"))

## NI11

ni11_raw <- readxl::read_excel(fs::path(get_ni_input_dir(), "NI11/age-standard-death-rates-24-tab4.xlsx"), sheet = "data for chartLA", range = "A3:AH23") %>%
  dplyr::rename(year = `...1`) %>%
  dplyr::filter(year != "All Persons") %>%
  dplyr::mutate(year = as.integer(year)) %>%
  dplyr::filter(dplyr::between(year, 2019, 2025)) %>%
  tidyr::pivot_longer(cols = c(2:34),
                      names_to = "partnership",
                      values_to = "rate") %>%
  dplyr::mutate(rate = as.numeric(rate)) #added as was getting: Error in `dplyr::bind_rows()`:
#! Can't combine `..1$rate` <character> and `..2$rate` <double>

ni11_cs <- readxl::read_excel(fs::path(get_ni_input_dir(), "NI11/age-standard-death-rates-24-cands.xlsx"), sheet = "Under 75", range = "A5:B23", col_names = c("year", "rate")) %>%
...
arrow::write_parquet(ni11, fs::path(get_ni_output_dir(), "ni11_excel_output_may_26.parquet"))
arrow::write_parquet(ni11_tableau, fs::path(get_ni_output_dir(), "ni11_tableau_output_may_26.parquet"))


ni11_raw %>%
  mutate(rate_num = suppressWarnings(as.numeric(rate))) %>%
  filter(is.na(rate_num)) %>%
  distinct(rate)
...
arrow::write_parquet(ni16_with_scot, fs::path(get_ni_output_dir(), "ni16_tableau_output_jul_26.parquet"))
...
arrow::write_parquet(ni16_final, fs::path(get_ni_output_dir(), "ni16_excel_output_jul_26.parquet"))

## Make a dataset without NI19

#prev_update <- arrow::read_parquet(fs::path(get_ni_output_dir(), "previous_mi_output.parquet"))
prev_update <- readxl::read_xlsx(fs::path(get_ni_excel_output_dir(), "spreadsheet_output_may_2026.xlsx"))

## Only required when new survey is released every 2 years
ni1_9 <- arrow::read_parquet(fs::path(get_ni_output_dir(), "NI1_to_NI9_202526_spreadsheet_output.parquet"))
...
current_raw <- dplyr::bind_rows(
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni11_excel_output_may_26.parquet")) %>%
    dplyr::mutate(year = as.character(year),
                  rate = as.numeric(rate)),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni12_excel_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni13_excel_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni14_excel_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni15_excel_output_jul_26.parquet")) %>%
    dplyr::mutate(ind_no = 15L),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni16_excel_output_jul_26.parquet")),
...
writexl::write_xlsx(final_output, fs::path(get_ni_excel_output_dir(), "spreadsheet_output_jul_2026_temp.xlsx"))

## NI19

ni19 <- calculate_ni19(file_from_dd_team = "obd_201516to202526_Q4_data") %>%
  dplyr::mutate(partnership = standardise_partnership_names(partnership))
...
#
#writexl::write_xlsx(ni19_excel, fs::path(get_ni_output_dir(), "ni19_excel_output_may_26.xlsx"))
#writexl::write_xlsx(ni19_tableau, fs::path(get_ni_output_dir(), "ni19_tableau_output_may_26.xlsx"))



arrow::write_parquet(ni19_excel, fs::path(get_ni_output_dir(), "ni19_excel_output_jul_26.parquet"))
arrow::write_parquet(ni19_tableau, fs::path(get_ni_output_dir(), "ni19_tableau_output_jul_26.parquet"))

full_data <- readxl::read_xlsx(fs::path(get_ni_excel_output_dir(), "spreadsheet_output_jul_2026_temp.xlsx"),
...
writexl::write_xlsx(final_data, fs::path(get_ni_excel_output_dir(), "spreadsheet_output_jul_2026.xlsx"))
...
new_raw <- dplyr::bind_rows(
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni11_tableau_output_may_26.parquet")) %>%
    dplyr::mutate(year = as.character(year),
                  rate = as.numeric(rate),
                  scotland = as.numeric(scotland),
                  year = format_financial_year(year, type = "single_year")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni12_tableau_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni13_tableau_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni14_tableau_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni15_tableau_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni16_tableau_output_jul_26.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni18_2024_tableau_output.parquet")),
  arrow::read_parquet(fs::path(get_ni_output_dir(), "ni19_tableau_output_jul_26.parquet"))
) %>%
  dplyr::mutate(year = as.character(year)) %>%
  dplyr::mutate(partnership = standardise_partnership_names(partnership)) %>%
  dplyr::mutate(true_value = dplyr::case_when(
...
old_raw <- read.csv(fs::path(get_ni_tableau_output_dir(), "tableau_output_june_2025.csv")) %>%
  rename(
...
ni17 <- ni17 %>% dplyr::left_join(lookup, by = c("partnership" = "ca2019name"))
ni17 <- rename(ni17, locality = hscp_locality)
ni17 <- ni17 %>% dplyr::filter(year == "2025/26")
ni17 <- ni17 %>%
  dplyr::mutate(
    UpperCI = NA_real_,
    LowerCI = NA_real_,
...
ni1_9 <- arrow::read_parquet(
  fs::path(get_ni_output_dir(), "NI1_to_NI9_202526_tableau_output.parquet")
) %>%
  dplyr::rename(
    rate    = value,
    UpperCI = upper_ci,
...
#prev_update <- dplyr::bind_rows(prev_update, ni1_9, ni17)

common_cols <- c(
  "indicator","year","time_period","partnership","locality",
  "denominator","rate","scotland","numerator","LA_Code",
  "UpperCI","LowerCI"
)
...
add_missing <- function(df) {
  ...
  df %>% dplyr::select(all_of(common_cols))
}

new_raw <- add_missing(new_raw)
old_raw <- add_missing(old_raw)
ni1_9  <- add_missing(ni1_9)
ni17   <- add_missing(ni17)


ni1_9 <- ni1_9 %>% dplyr::mutate(year = as.character(year))
ni17  <- ni17  %>% dplyr::mutate(year = as.character(year))

output <- dplyr::bind_rows(new_raw, old_raw, ni1_9, ni17) %>%
  dplyr::filter(
    partnership != "Moray" |
      (partnership == "Moray" & locality %in% lookup_moray$hscp_locality) |
      (partnership == "Moray" & locality == "All")
  )
output <- output %>%
  dplyr::mutate(
    UpperCI = dplyr::if_else(!is.na(UpperCI), 1, NA_real_),
    LowerCI = dplyr::if_else(!is.na(LowerCI), 1, NA_real_)
  ) %>%
  dplyr::select(
    indicator, year, time_period, partnership, locality,
    denominator, rate, scotland, numerator, LA_Code,
...
head(output %>% filter(Indicator1 %in% paste0("NI", 1:9), Year1 == "2025/26"))
...
#arrow::write_parquet(new, fs::path(get_ni_tableau_output_dir(), "tableau_output_may_2026.parquet"))
#write_csv(new, fs::path(get_ni_tableau_output_dir(), "tableau_output_may_2026.csv"))


arrow::write_parquet(output %>%
                       dplyr::rename(indicator   = Indicator1, year        = Year1,
                                     time_period = Data1,
                                     partnership = Partnership1,
                                     locality    = Locality,
                                     value       = Value
                       ),
                     fs::path(get_ni_tableau_output_dir(), "tableau_output_jul_2026.parquet")
)
...
readr::write_csv(
  ...
  fs::path(get_ni_tableau_output_dir(), "tableau_output_jul_2026.csv"),
  na = ""
)
...
#NI20 <- read_csv(fs::path(get_ni_tableau_output_dir(), "tableau_output_may_2026.csv"))
...
# Extract NI20 rows from the clean output
NI20 <- output %>%
  dplyr::filter(
    Indicator1 == "NI20",
    Locality == "All",
    Year1 %in% c("2016/17", "2017/18", "2018/19", "2019/20")
  )

# Remove NI20 from main dataset
output_l2 <- output %>%
  dplyr::filter(Indicator1 != "NI20")

# Bind corrected NI20 back in
output_l2 <- dplyr::bind_rows(output_l2, NI20)

# Write L2
haven::write_sav(
  output_l2,
  fs::path(get_ni_tableau_output_dir(),
           "NI Tableau Final-L2-September-2020.sav")
)

readr::write_csv(
  output_l2,
  fs::path(get_ni_tableau_output_dir(), "NI L2.csv"),
  na = ""
)
...
# Level 3 – extract NI20 from the clean output
NI20_l3 <- output %>%
  dplyr::filter(
    Indicator1 == "NI20",
    Year1 %in% c("2016/17", "2017/18", "2018/19", "2019/20")
...
output_l3 <- output_l3 %>%
  dplyr::left_join(postcode_lookup, by = "Partnership1") %>%
  dplyr::mutate(
    hb2019 = dplyr::case_when(
      Partnership1 == "Clackmannanshire and Stirling" ~ "S08000019",
      Partnership1 == "Scotland" ~ "S92000003",
      Partnership1 == "Undetermined" ~ "S08200003",
      TRUE ~ hb2019
...
haven::write_sav(
  output_l3,
  fs::path(get_ni_tableau_output_dir(),
           "NI Tableau Final-L3-September-2020.sav")
)

# Final Level 3 CSV
readr::write_csv(
  output_l3,
  fs::path(get_ni_tableau_output_dir(), "NI_LV3.csv"),
  na = ""
)
